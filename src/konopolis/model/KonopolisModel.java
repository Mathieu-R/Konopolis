...
public class KonopolisModel extends Observable {
  ...
  private String DB_URL = "jdbc:mysql://localhost:3306/konopolis?autoReconnect=true&useSSL=false"; // auto reconnection
                                                                                                   // and no ssl
                                                                                                   // connection (not
                                                                                                   // prod ready)
  ...
  private ArrayList<Show> shows_al = new ArrayList<Show>(); // ArrayList of Shows => contain every instance of shows for
                                                            // a specific movie
  private ArrayList<Customer> customers_al = new ArrayList<Customer>(); // ArrayList of Customers to be able to manage
                                                                        // them
   ...
   *
   * @return HashMap that contains the id => key and the title => value of the
   *         movies
   * @throws SQLException
   */
  ...
  public synchronized Movie retrieveMovie(int movie_id) {
    ...
    try {
      while (rs.next()) { // While there're still results
        ...
        // Split the string of casting ("act1, act2, act3") into Array ["act1", "act2",
        // "act3"]
        ArrayList<String> casting = new ArrayList<String>(Arrays.asList(rs.getString("casting").split(","))); // Several
                                                                                                              // actors
        ArrayList<String> genres = new ArrayList<String>(Arrays.asList(rs.getString("genres").split(","))); // Several
                                                                                                            // genres
        ...
        // movies_al.clear(); // A changer, il faudrait vérifier si le film existe déjà,
        // dans ce cas ne pas faire la requête une seconde fois
        if (!movies_al.contains(movie))
          movies_al.add(movie); // if the movie is not present, we add it
  ...
  /**
   * Retrieve all the customers of a [movie] that happens in a [room] at a time
   * given [show_start]
   *
   * @param room_id
   * @param movie_id
  ...
  public synchronized void retrieveCustomers(int room_id, int movie_id, LocalDateTime show_start) {
    ...
    try {
      while (rs.next()) { // While there're still results
        ...
        // Look for the right Room => create an instance of customer and add it to the
        // customers ArrayList
        for (Room room : rooms_al) {
          if (room.getId() == room_id) { // Retrieve the right room
            try {
              customers_al.add(new Customer(column, row, room, customer_type, customer_id)); // The instance of customer
                                                                                             // book the seat
  ...
  /**
   * Return the id of the customer type (to be able to add the customer to the db
   * (relationnal db))
   *
  ...
  /**
   * Retrieve the id movie_room_id of a customer (to be able to add a customer to
   * the db (relationnal db))
   *
  ...
  /**
   * retrieve the genre_id based on a genre, if the genre doesn't exist yet, we
   * create it and send back it id;
   *
  ...
  /**
   * retrieve the cast_id based on a actor, if the actor doesn't exist yet, we
   * create it and send back it id;
   *
   ...
   * @param shows_start, /!\ IMPORTANT => ArrayList of type Date and not
   *                     LocalDateTime (LocalDateTime cannot be inserted in DB),
   *                     the starts of every show
  ...
  public synchronized void addGenres(int movie_id, ArrayList<String> genres) {
    ...
    try {
      ...
      for (String genre : genres) { // For Each genre
        addGr = conn.prepareStatement("INSERT INTO tbmoviesgenres(movie_id, genre_id) " // Add the genre associated to
                                                                                        // the movie in the db
            + "VALUES (?, ?)");
  ...
  public synchronized void addShows(int movie_id, int room_id, ArrayList<LocalDateTime> shows_start) {
    ...
    try {
      ...
      for (LocalDateTime show_start : shows_start) { // For Each genre
        addSh = conn.prepareStatement("INSERT INTO tbmoviesrooms(movie_id, room_id, show_start) " // Add the genre
                                                                                                  // associated to the
                                                                                                  // movie in the db
            + "VALUES (?, ?, ?)");
  ...
  public synchronized void addCasting(int movie_id, ArrayList<String> casting) {
    ...
    try {
      ...
      for (String actor : casting) { // For Each genre
        addCs = conn.prepareStatement("INSERT INTO tbmoviescasts(movie_id, cast_id) " // Add the genre associated to the
                                                                                      // movie in the db
            + "VALUES (?, ?)");
  ...
  private synchronized LocalDateTime stringToLocalDateTime(String show) {
    ...
    // start of the show
    // LocalDate show_start_date = LocalDate.parse(show, format); // Date
    // (yyyy-MM-dd)
    // LocalTime show_start_time = LocalTime.parse(show, format); // Time (hh:mm:ss)
    // LocalDateTime show_start = LocalDateTime.of(show_start_date,
    // show_start_time); // Parse in LocalDateTime
