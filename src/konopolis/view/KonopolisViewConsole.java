...
public class KonopolisViewConsole extends KonopolisView implements Observer{
    ...
    private void bookMovie() {
        ...
        do {
            ...
            do {
                try {
                    show("Sélectionner votre place en x,y (siÃ¨ge, rangée)");
        ...
        for (Map.Entry<String, Double> bookingEntry: control.getBooking().entrySet()) {
            show("> " + bookingEntry.getKey() + " - " + Math.round(bookingEntry.getValue() * 100.0) / 100.0 + "â‚¬");
        }
        
        // Total to pay
        show("TOTAL A PAYER : " + Math.round(control.getTotal() * 100.0) / 100.0 + "â‚¬");
    ...
    private void addMovie() {
        ...
        for (Room room : control.getRooms_al()) { // Show every room
            show(room.getId() + ") " + room.getRows() + " rangées - " + room.getSeatsByRow() + " siÃ¨ges par rangée - " + room.getTotSeats() + " places");
        }
    ...
    private void showTypeOfPeople() {
        show("Quel type de personne Ãªtes-vous ?\n");
