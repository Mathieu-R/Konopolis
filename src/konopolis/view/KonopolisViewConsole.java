...
public class KonopolisViewConsole extends KonopolisView implements Observer {
    ...
    do {
      ...
      do {
        try {
          ...
          // LocalDateTime show_start; // temp variable that will contain the beginning
          // date of the show
      ...
      } while ((!moreBooking.toLowerCase().equals("oui")) && (!moreBooking.toLowerCase().equals("non"))); // while the
                                                                                                          // user type
                                                                                                          // something
                                                                                                          // else than
                                                                                                          // 'oui' or
  ...
  private void addMovie() {
    ...
    for (Room room : control.getRooms_al()) { // Show every room
      show(room.getId() + ") " + room.getRows() + " rangées - " + room.getSeatsByRow() + " sièges par rangée - "
          + room.getTotSeats() + " places");
    }
   ...
   * @return, an ArrayList of LocalDateTime that contains all the shows entered by
   * the user
   */
  private ArrayList<LocalDateTime> enterDate() {
    ...
    do { // The user can enter a date of show
      ...
      } while ((!moreDate.toLowerCase().equals("oui")) && (!moreDate.toLowerCase().equals("non"))); // if the user
                                                                                                    // doesn't write
                                                                                                    // 'oui' or 'non'
  ...
  private void showMoviesList() {
    // The number in the list (1,2,3) is not equals the movie_id but the position of
    // the movie_id and its title in the LinkedHashMap (+ 1)
    int movieListNumber = 0;
    show("Liste des films:");
    int index = 1;
    for (Map.Entry<Integer, String> movieEntry : control.retrieveAllMoviesTitles().entrySet()) { // for every movie in
                                                                                                 // the HashMap
    ...
    do { // the user can choose one
      try {
        movieListNumber = sc.nextInt(); // get the number of the list (which is equals the position of the movie in the
                                        // LinkedHashMap + 1)
        movie_id = (new ArrayList<Integer>(control.getMoviesTitles().keySet())).get(movieListNumber - 1); // get the
                                                                                                          // right id
                                                                                                          // according
                                                                                                          // its
                                                                                                          // position in
    ...
    } while (movieListNumber < 1 || movieListNumber > control.getMoviesTitles().size()); // while the int typed is not
                                                                                         // in the list of movie
    ...
    for (Movie movie : control.getMovies_al()) { // for every movie
      if (movie.getId() == movie_id) { // look for the right one
        for (int i = 0; i < movie.getShows().size(); i++) { // for every shows of this movie
          show((i + 1) + ") " + control.dateInFrench(movie.getShows().get(i).getShow_start())); // Show the list of
                                                                                                // shows
        }
        ...
        for (ArrayList<Seat> seatsRow : room.getSeats()) { // For Each row
          ...
          if (rowIndex == 1) {
            ...
            for (Seat seat : seatsRow) {
              if (seatIndex < 10)
                showInline(seatIndex + "  "); // Show the column above the map, two spaces for number < 10
              else
                showInline(seatIndex + " "); // one space for number > 10
              seatIndex++;
            }
            showInline("\n"); // Turn back to the line
          }
          if (rowIndex < 10)
            showInline(rowIndex + "  "); // Show the row at the right of the map, two spaces for number < 10
          else
            showInline(rowIndex + " "); // one space for number > 10
          for (Seat seat : seatsRow) { // For Each seat
            if (seat.isTaken())
              showInline("[X]"); // If the seat is taken
            else
              showInline("[O]"); // Otherwise, if the seat is available
          }
  ...
  /**
   * Show every type of people available (VIP, Senior, Student,..) to apply a
   * reduction
